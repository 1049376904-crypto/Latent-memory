...
| 自建前端（`--client generic`） | **已有一例维护者生产实测跑通**：小克Cat 采用 Node 宿主＋Claude Code 常驻进程＋Latent HTTP；判据是五工具握手成功，真实主聊天主动调用 `latent_search`，并同时命中两段目标原文。采集条件为 2026.08.15、VPS 生产环境、Latent `656a044`、Voyage `voyage-3.5`、182 个 Markdown／869 个切块。⚠ 这只证明“借宿主引擎”的这一种自建形态；换成你自己的请求拼装，注入契约仍要逐条自验 |
