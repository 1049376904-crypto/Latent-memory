...
启发式。**（这两条当天早些时候已写进 changelog 与任务卡，现已在原处更正。）
  ...
  ③ 顺带还有第三个：第一版填充块用 `_TOPICS[i % 25]` 生成，186 块里 7 块共用一个
     标题、正文高度雷同；近似重复的块会成群结队地以微弱相似度挤进 topN。已改成
     话题×地点×细节三维错开，186 块两两不同。
...
**修好仪器之后，剩下一个真结论**：`linked`（跨窗口关联）**在真实规模下基本失效**，
**规模一上来，两类检索一起掉**：`linked`（跨窗口关联）R0.60 → R0.40，
...
BASELINE = {
    ...
    "cold": {
        "literal":    {"recall": 1.00, "mrr": 0.90},
        "paraphrase": {"recall": 1.00, "mrr": 1.00},
        "linked":     {"recall": 0.60, "mrr": 0.21},
    ...
    "established": {
        ...
        "paraphrase": {"recall": 0.75, "mrr": 0.75},
        # **这两个数是当前最重要的一个事实**：跨窗口关联在真实规模下基本失效
        # （cold 0.60 → established 0.20，规模越大越差）。图谱边建对了、主题词
        # 也没被 max_df 挡掉，但图谱路在 RRF 里只有一票，压不过那些在两条词面路
        # 里都排前几名的闲词块。**没有据此改检索层**——理由见 docstring 末尾。
        "linked":     {"recall": 0.40, "mrr": 0.13},
        "absent":     {"correct_empty": 1.00},
    },
}
TOPN = 5
...
def _selftest():
    ...
    print(f"selftest ok（回归集：{len(idx.chunks)} 块语料 / {len(CASES)} 条查询，"
          f"8 项断言：两档不低于基线 / 门槛没松 / 实体槽增益状态 / 消融走真实路径 / 更正闭环 / 语料代表性三道闸）")
